...
commanderProgram.version(JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json'), 'utf8')).version);
...
if (selectedOptions.init) {
  ...
  try {
    const configPath = (selectedOptions.configFile as string) || getDefaultConfigFilePath(dirPath);
